...
function App() {
	...
	const [viewMode, setViewMode] = useState<'cards' | 'comparison' | 'detailed'>('cards');
	...
	const getFastestAlgorithm = () => {
		...
		return successful.length > 0 ? 
			successful.sort((a, b) => a.execution_time_ms - b.execution_time_ms)[0] : null;
	};
	...
	const getAccuracyLeader = () => {
		...
		const successful = result.results.filter(r => r.success);
		return successful.length > 0 ? 
			successful.sort((a, b) => b.precision - a.precision)[0] : null;
	};
				...
				<div className="insights-grid">
					<div className="insight-card highlight">
						...
						<div className="insight-content">
							...
							<small>{best ? `${(best.precision * 100).toFixed(1)}% precision, ${best.execution_time_ms.toFixed(2)}ms` : ''}</small>
						</div>
					</div>
					<div className="insight-card">
						...
						<div className="insight-content">
							...
							<small>{fastest ? `${fastest.execution_time_ms.toFixed(2)}ms` : ''}</small>
						</div>
					</div>
					<div className="insight-card">
						<div className="insight-icon">🎯</div>
						<div className="insight-content">
							...
							<small>{mostAccurate ? `${(mostAccurate.precision * 100).toFixed(1)}% precision` : ''}</small>
						</div>
					</div>
					<div className="insight-card">
						<div className="insight-icon">✅</div>
						<div className="insight-content">
							<h4>Success Rate</h4>
							<p>{successfulAlgorithms}/{result.results.length}</p>
							<small>{((successfulAlgorithms / result.results.length) * 100).toFixed(0)}% successful</small>
						</div>
					</div>
				</div>
			</div>
						...
						<tbody>
							{ranking.map((algorithm, index) => {
								const performanceScore = algorithm.success ? 
									((algorithm.precision * 0.7) + ((1 / algorithm.execution_time_ms) * 1000 * 0.3)) : 0;
								const isTop = index === 0 && algorithm.success;
								
								return (
									<tr key={algorithm.algorithm_name} className={isTop ? 'top-performer' : ''}>
										<td>
											<span className={`rank-badge ${isTop ? 'rank-gold' : index === 1 ? 'rank-silver' : index === 2 ? 'rank-bronze' : ''}`}>
												{isTop ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `#${index + 1}`}
											</span>
										</td>
										<td className="algorithm-name">{algorithm.algorithm_name}</td>
										<td>
										...
										<td>
											<span className="time-value">{algorithm.execution_time_ms.toFixed(4)} ms</span>
										</td>
										<td>
											<span className={`status-badge ${algorithm.success ? 'success' : 'failed'}`}>
											...
											<div className="score-cell">
												<div className="score-bar">
													<div 
														...
														style={{ width: `${Math.min(performanceScore * 10, 100)}%` }}
													></div>
												</div>
		...
		return (
			<div className="detailed-view">
				...
				{result.results.map((algorithm) => (
					<div key={algorithm.algorithm_name} className="detailed-algorithm-card">
						<div className="algorithm-header">
							...
							<div className="algorithm-badges">
								...
								{algorithm.precision === 1.0 && <span className="badge perfect">Perfect</span>}
								{algorithm === getFastestAlgorithm() && <span className="badge fastest">Fastest</span>}
								{algorithm === getAccuracyLeader() && <span className="badge accurate">Most Accurate</span>}
							</div>
						</div>
						...
						<div className="algorithm-metrics">
							...
							<div className="metric">
								...
								<div className="metric-value">
									<span className="time-large">{algorithm.execution_time_ms.toFixed(4)}</span>
							...
							<div className="metric">
								...
								<div className="metric-value">
									<span className="matches-count">
										{algorithm.distances_and_matches.filter(m => m.is_match).length}
									</span>
									<span className="matches-total">/ {algorithm.distances_and_matches.length}</span>
								</div>
							</div>
						</div>
								...
								{algorithm.distances_and_matches.map((match, index) => (
									...
									>
										<div className="distance-info">
											...
											<span className="distance-value">{match.distance_squared.toFixed(0)}</span>
											<span className={`match-indicator ${match.is_match ? 'match' : 'no-match'}`}>
												{match.is_match ? '✓' : '✗'}
											</span>
										</div>
				...
				<div className="form-sidebar">
					<div className="form-section">
						...
						<div className="button-group">
							<button type="button" onClick={submitCompareRequest} disabled={loading || !queryIndex || !k}>
								{loading ? (
									<>
						...
						<div className="results-section">
							...
							<div className="result-summary">
								...
								<div className="summary-item">
									...
									<span className="summary-value">{((result.results.filter(r => r.success).length / result.results.length) * 100).toFixed(0)}%</span>
								</div>
							</div>
							...
							<div className="results-content">
								{viewMode === 'cards' && (
									<div className="algorithms-grid">
										{result.results.map((algorithm) => (
											<div key={algorithm.algorithm_name} className="algorithm-card">
												...
												<div className="distances-section">
													<h4>Top {algorithm.distances_and_matches.length} Results</h4>
